		...
		<div class="hidden items-center space-x-8 text-sm font-medium md:flex">
			{#each navItems as item}
				...
				>
					{item.label}
					<span
						class="absolute -bottom-1 left-0 h-0.5 w-0 bg-red-600 transition-all group-hover:w-full"
					></span>
				</button>
			{/each}

			<button
				class="cursor-pointer rounded-xl bg-linear-to-r from-red-600 to-rose-500 px-5 py-2.5 font-semibold text-white shadow-sm transition-all duration-300 hover:scale-105 hover:shadow-lg"
			>
				Join Now
			</button>
		</div>
