<footer id="footer" class="mt-auto border-t border-gray-200 bg-white px-6 py-9 md:px-8">
	<div class="mx-auto grid max-w-6xl gap-10 md:grid-cols-4">
		<div>
			<h2
				class="mb-4 bg-linear-to-r from-red-600 to-rose-400 bg-clip-text text-xl font-bold text-transparent"
			>
				MandarinPals
			</h2>
			<p class="text-sm leading-relaxed text-gray-500">
				Platform belajar Mandarin yang membantu kamu berkembang dari nol sampai percaya diri
				berbicara.
			</p>
		</div>

		<div>
			<h4 class="mb-4 font-semibold text-gray-800">Program</h4>
			<ul class="space-y-2 text-sm text-gray-500">
				<li>Beginner Class</li>
				<li>Conversation Class</li>
				<li>HSK Preparation</li>
				<li>Private Class</li>
			</ul>
		</div>

		<div>
			<h4 class="mb-4 font-semibold text-gray-800">Kontak</h4>
			<ul class="space-y-2 text-sm text-gray-500">
				<li>Email: hello@mandarinpals.com</li>
				<li>WhatsApp: +62 812-xxxx-xxxx</li>
				<li>Instagram: @mandarinpals</li>
			</ul>
		</div>
	</div>

	<div
		class="mx-auto mt-12 flex max-w-6xl flex-col items-center justify-between border-t border-gray-200 pt-6 text-sm text-gray-400 md:flex-row"
	>
		<p>© 2026 MandarinPals. All rights reserved.</p>

		<!-- svelte-ignore a11y_invalid_attribute -->
		<div class="mt-4 flex gap-4 md:mt-0">
			<a href="#" class="transition hover:text-red-600">Privacy</a>
			<a href="#" class="transition hover:text-red-600">Terms</a>
			<a href="#" class="transition hover:text-red-600">Support</a>
		</div>
	</div>
</footer>
